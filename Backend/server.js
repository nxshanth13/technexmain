...
console.log(email)
  pool.query("SELECT test_id FROM test_activity WHERE email=? AND status=false", [email], (err, result) => {
    if (err) {
      console.log(err);
...
app.post("/addscore/:id/:id2", (req, res) => {
  ...
  pool.query('UPDATE test_activity SET status = true, score = ? WHERE email = ? AND test_id = ?  ', [score,id2, id1], (err, result) => {
    if (err) {
      console.log(err);
    } else {
...
app.get("/userp/:email",(req,res)=>{
  const email = req.params.email;
  pool.query("select test_name, score,test_id from test_activity where email=? and status=true",[email],(err,result)=>{
    if(err){
      console.log(err);
    }
    else{
